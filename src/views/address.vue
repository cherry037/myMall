<template>
    <div>
            ...
            <div class="container">
                <div class="checkout-addr">
                    ...
                    <div class="page-title-normal checkout-title">
                        <h2><span>Shipping addresss</span></h2>
                    </div>
                    <div class="addr-list-wrap">
                        <div class="addr-list">
                            <ul>
                                <li v-for="(item, index) in addressListFilter" :key="index" :class="{'check': checkIndex === index}" @click="selectItem(item, index)">
                                    <dl>
                                        <dt>{{item.userName}}</dt>
                                    ...
                                    <div class="addr-opration addr-del">
                                        <a href="javascript:void(0);" class="addr-del-btn" @click="delAddressConfirm(item.addressId)">
                                            <svg class="icon icon-del"><use xlink:href="#icon-del"></use></svg>
                                        </a>
                                    </div>
                                    <div class="addr-opration addr-set-default">
                                        <a href="javascript:void(0);" class="addr-set-default-btn" v-if="!item.isDefault" @click="setDefault(item.addressId)"><i>Set default</i></a>
                                    </div>
                        ...
                        <div class="shipping-addr-more">
                            <div class="addr-more-btn up-down-btn" href="javascript:void(0);" @click="expand" :class="{'open': limit>3}">
                                more
                                <i class="i-up-down">
        ...
        <modal v-if="delModalShow">
            ...
            <div slot="btnGroup">
                <a href="javascript:void(0);" class="btn btn--m" @click="delModalShow = false">取消</a>
                <a href="javascript:void(0);" class="btn btn--red" @click="delAddress">确定</a>
            </div>
        </modal>
    ...
    methods: {
        init () {
            axios.get('/users/addressList').then(res => {
                const addressList = res.data.result.addressList
                this.addressList = addressList || []
                addressList.forEach((item, index) => {
                    if (item.isDefault) {
                        this.selectAddressId = item.addressId
                        if (index > 2) {
                            this.limit = addressList.length
                        }
