<template>
    <div>
                            ...
                            <li v-for="(item, index) in cartList" :key="index">
                                <div class="cart-tab-1">
                                    <div class="cart-item-check">
                                        <a href="javascript:void(0);" class="checkbox-btn item-check-btn" :class="{'check': item.checked === 1}" @click="editCart('checked', item)">
                                ...
                                <div class="cart-tab-5">
                                    <div class="cart-item-operation">
                                        <a href="javascript:void(0);" class="item-edit-btn" @click="delCartConfirm(item)">
        ...
        <modal v-show="modalShow" @close="closeModal">
            <p slot="message">
                确认删除此数据？
            </p>
            <div slot="btnGroup">
                <a class="btn btn--m" href="javascript:void(0);" @click="modalShow =false">取消</a>
                <a href="javascript:void(0);" class="btn btn--m btn--red" @click="delCart">确认</a>
            </div>
        </modal>
    ...
    methods: {
        ...
        delCart () {
            axios.post('/users/delCart', {
                productId: this.delData.productId
            }).then(res => {
                if (res.data.status === '0') {
                    this.init()
